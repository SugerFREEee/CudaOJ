...
def get_2d_autotune_config():
    """为 2-D 网格的矩阵乘法 kernel 提供自动调优配置"""
    return [
        # 大尺寸块配置
        ...
        # 针对小矩阵的配置
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_N': 32, 'BLOCK_SIZE_K': 64}, num_stages=4, num_warps=2),
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_N': 16, 'BLOCK_SIZE_K': 32}, num_stages=5, num_warps=1),
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_N': 32, 'BLOCK_SIZE_K': 32}, num_stages=5, num_warps=1)
    ]

def get_1d_autotune_config():
    """为 1-D 网格的矩阵乘法 kernel 提供自动调优配置（无 L2 优化）"""
    return [
        # 大尺寸块配置
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_K': 64}, num_stages=3, num_warps=8),
        triton.Config({'BLOCK_SIZE_M': 256, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 64}, num_stages=3, num_warps=8),
